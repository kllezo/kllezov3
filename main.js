    ...
    if (phoneSettled) {
      ...
      if (state.state === 'customer_typing') {
        ...
        if (state.timer >= state.charDelay) {
          ...
          // 25% faster overall conversation speed: base typing speed 9-18ms

          const baseSpeed = 0.009 + Math.random() * 0.009;

          const lastChar = state.inputText[state.inputText.length - 1];

          if (lastChar === ' ') {

            state.charDelay = 0.02 + Math.random() * 0.02;

          } else if (lastChar === ',' || lastChar === '.' || lastChar === '?' || lastChar === '!') {

            state.charDelay = 0.04 + Math.random() * 0.04;
          ...
          if (state.charIndex >= activeMsg.text.length) {
            ...
            state.targetDelay = 0.15 + Math.random() * 0.15; // 150 - 300ms pause
        ...
        if (state.scriptIndex >= CONVERSATION_SCRIPT.length) {
          ...
          state.targetDelay = 0.55 + Math.random() * 0.15; // 0.55 to 0.70s typing indicator (faster)
        ...
        if (state.timer >= state.targetDelay) {
          ...
          state.targetDelay = 0.55; // 0.55s delay before next user message starts
...
lightWebsites.intensity = 0.0; // Disabled to remove the bright white hotspot on the glossy floor

scene.add(lightWebsites);



// ── WEBSITE ARRIVAL ARCHITECTURE: SHARED WORLD ANCHOR ──

const WebsiteArrivalAnchor = new THREE.Group();

WebsiteArrivalAnchor.name = 'WebsiteArrivalAnchor';

WebsiteArrivalAnchor.position.set(0, -11.96, -185.0);

scene.add(WebsiteArrivalAnchor);
...
WebsiteArrivalAnchor.add(PlatformRoot);



const WalkwayRoot = new THREE.Group();

WalkwayRoot.name = 'WalkwayRoot';

WebsiteArrivalAnchor.add(WalkwayRoot);



const CameraLanding = new THREE.Object3D();

CameraLanding.name = 'CameraLanding';

CameraLanding.position.set(0.0, 5.46, 15.0); // local offset (world Y = -6.5, Z = -170.0)

WebsiteArrivalAnchor.add(CameraLanding);
...
WebsiteArrivalAnchor.add(ArrowRoot);



const EntranceLighting = new THREE.Group();

EntranceLighting.name = 'EntranceLighting';

WebsiteArrivalAnchor.add(EntranceLighting);



// Soft gold entrance light (gentle ambient glow, no hot bloom)

const entrancePointLight = new THREE.PointLight(0xFFD27D, 0.0, 30);

entrancePointLight.position.set(0, 4.0, 0); // 4 units above the platform base

EntranceLighting.add(entrancePointLight);
...
const CAM_PATH = new THREE.CatmullRomCurve3([
  ...
  new THREE.Vector3(0,   4.5, -170), // t=0.28  Entering website corridor (Camera lands before it at Z=-170)

  new THREE.Vector3(0,   4.5, -262), // t=0.32  Website Row 2

  new THREE.Vector3(0,   4.5, -354), // t=0.36  Website Row 3

  new THREE.Vector3(0,   4.5, -445), // t=0.40  End of Websites walkway (Website exit Z=-445)

  new THREE.Vector3(0,   4.5, -532), // t=0.44  Transition 1 travel

  new THREE.Vector3(0,   4.5, -620), // t=0.48  Calling entry — parked at z=-620

  new THREE.Vector3(0,   4.5, -620), // t=0.52  Parked at Calling

  new THREE.Vector3(0,   4.5, -620), // t=0.55  Parked at Calling

  new THREE.Vector3(0,   4.5, -620), // t=0.58  Calling hold

  new THREE.Vector3(0,   4.5, -635), // t=0.61  Calling hold end / transition 2 starts (Z = -455 - 180)

  new THREE.Vector3(0,   4.5, -690), // t=0.65  Transition 2 travel (Z = -510 - 180)

  new THREE.Vector3(0,   4.5, -740), // t=0.68  Texting entry — parked at z=-740 (Z = -560 - 180)

  new THREE.Vector3(0,   4.5, -740), // t=0.72  Parked at Texting

  new THREE.Vector3(0,   4.5, -740), // t=0.75  Texting hold

  new THREE.Vector3(0,   4.5, -740), // t=0.78  Exit Texting

  new THREE.Vector3(0,   4.5, -740), // t=0.80  Website Bot hold

  new THREE.Vector3(0,   4.5, -815), // t=0.84  Transition 3 travel space (Z = -635 - 180)

  new THREE.Vector3(0,   4.5, -870), // t=0.88  Ecosystem entry (Z = -690 - 180)

  new THREE.Vector3(0,   4.5, -870), // t=0.93  Ecosystem flat framing

  new THREE.Vector3(0,   4.5, -870), // t=1.00  Final framing
...
const LOOK_PATH = new THREE.CatmullRomCurve3([
  ...
  new THREE.Vector3(0,   4.5, -210),  // t=0.28  Look ahead into corridor

  new THREE.Vector3(0,   4.5, -300),  // t=0.32  Through Row 2

  new THREE.Vector3(0,   4.5, -390),  // t=0.36  Through Row 3

  new THREE.Vector3(0,   4.5, -620),  // t=0.40  Look towards Calling entry

  new THREE.Vector3(0,   4.5, -625),  // t=0.44  Transition 1 travel

  new THREE.Vector3(0,   4.5, -634),  // t=0.48  Look at Calling screens (Z = -454 - 180)

  new THREE.Vector3(0,   4.5, -634),  // t=0.52

  new THREE.Vector3(0,   4.5, -634),  // t=0.55

  new THREE.Vector3(0,   4.5, -634),  // t=0.58

  new THREE.Vector3(0,   4.5, -660),  // t=0.61  Transition 2 travel starts (Z = -480 - 180)

  new THREE.Vector3(0,   4.5, -720),  // t=0.65  Transition 2 travel mid (Z = -540 - 180)

  new THREE.Vector3(0,   4.5, -752),  // t=0.68  Look at Texting phone (Z = -572 - 180)

  new THREE.Vector3(0,   4.5, -752),  // t=0.72

  new THREE.Vector3(0,   4.5, -752),  // t=0.75

  new THREE.Vector3(0,   4.5, -752),  // t=0.78

  new THREE.Vector3(0,   4.5, -752),  // t=0.80  Look at texting phone

  new THREE.Vector3(0,   4.5, -860),  // t=0.84  Look ahead to ecosystem (Z = -680 - 180)

  new THREE.Vector3(0,   4.5, -920),  // t=0.88  Ecosystem framing (logo at Z = -740 - 180)

  new THREE.Vector3(0,   4.5, -920),  // t=0.93

  new THREE.Vector3(0,   4.5, -920),  // t=1.00  Final frame
...
const pTopLeft = new THREE.Vector3(-25, 18.5, -920);

const pCenter = new THREE.Vector3(0, 4.5, -920);

const pTopRight = new THREE.Vector3(25, 18.5, -920);

const pBottomRight = new THREE.Vector3(25, -9.5, -920);

const pBottomLeft = new THREE.Vector3(-25, -9.5, -920);
...
// Starts exactly at the Content Engine parked position (Z = -65.5) and terminates cleanly before WebsiteArrivalAnchor (Z = -170.0)
...
const ptsContentToWebsites = [
  ...
  new THREE.Vector3(5.5, -11.5, -170),        // Approach low, following platform direction

  new THREE.Vector3(2.5, -11.96, -170),       // Flatten naturally

  new THREE.Vector3(0.0, -6.5, -170.0)        // Terminate cleanly before the walkway
...
// Visual tube curve: elegant M-shaped flow behind the Calling cards (Z-shifted by -180 units)

const ptsWebsitesToCalling = [

  new THREE.Vector3(0.0, -12.0, -445.0),

  new THREE.Vector3(-1.0, 5.0, -465.0),

  new THREE.Vector3(-3.0, 4.2, -485.0),

  new THREE.Vector3(-6.0, 3.5, -505.0),

  new THREE.Vector3(-10.0, 2.5, -525.0),

  new THREE.Vector3(-14.0, 2.0, -545.0),

  new THREE.Vector3(-18.0, 1.5, -638.0),

  new THREE.Vector3(-15.0, 4.5, -638.0),

  new THREE.Vector3(-10.5, 2.0, -638.0),

  new THREE.Vector3(-5.25, 7.5, -638.0),

  new THREE.Vector3(0.0, 2.0, -638.0),

  new THREE.Vector3(5.25, 7.5, -638.0),

  new THREE.Vector3(10.5, 2.0, -638.0),

  new THREE.Vector3(16.5, 7.5, -638.0),

  new THREE.Vector3(22.0, 2.5, -638.0)
...
// Visual tube curve: elegant loops framing the Texting phone (Z-shifted by -180 units)

const ptsCallingToTexting = [

  new THREE.Vector3(22.0, 2.5, -638.0),

  new THREE.Vector3(14.0, -5.0, -665.0),

  new THREE.Vector3(6.0, -15.0, -685.0),

  new THREE.Vector3(-2.0, -18.0, -705.0),

  new THREE.Vector3(-8.0, -15.0, -720.0),

  new THREE.Vector3(-12.0, -5.0, -732.0),

  new THREE.Vector3(-7.5, -2.0, -738.0),

  new THREE.Vector3(-3.5, 0.1, -744.0),

  new THREE.Vector3(1.0, 1.2, -748.0),

  new THREE.Vector3(-2.5, 1.8, -748.0),

  new THREE.Vector3(-5.0, 2.8, -748.0),

  new THREE.Vector3(-2.5, 3.8, -748.0),

  new THREE.Vector3(2.0, 4.8, -748.0),

  new THREE.Vector3(5.0, 5.8, -748.0),

  new THREE.Vector3(3.0, 6.8, -748.0),

  new THREE.Vector3(-2.0, 7.8, -748.0),

  new THREE.Vector3(-5.0, 8.8, -748.0),

  new THREE.Vector3(-2.5, 9.8, -748.0),

  new THREE.Vector3(1.0, 11.0, -748.0)

];



// Points for Segment 5: AI Texting Agents to Ecosystem Center Logo (Z-shifted by -180 units)

const ptsTextingToEcosystem = [

  new THREE.Vector3(1.0, 11.0, -748.0),

  new THREE.Vector3(0.0, 5.0, -785.0),

  new THREE.Vector3(0.0, -10.0, -815.0),

  new THREE.Vector3(0.0, -32.0, -920.0),

  new THREE.Vector3(0.0, -18.0, -920.0),

  new THREE.Vector3(0.0, -6.0, -920.0),

  new THREE.Vector3(0.0, 4.5, -920.0)
...
const camWebsitesToCalling = new THREE.CatmullRomCurve3([
  new THREE.Vector3(0.0, -12.0, -445.0),  // Flush with floor at exit
  new THREE.Vector3(0.0, -5.0, -475.0),   // Rises up
  new THREE.Vector3(0.0, 1.0, -510.0),    // Continues rise
  new THREE.Vector3(0.0, 1.0, -540.0),    // Settles Calling Z
  new THREE.Vector3(0.0, 1.0, -570.0)     // Final end point
], false, 'centripetal');

const camCallingToTexting = new THREE.CatmullRomCurve3([
  new THREE.Vector3(0.0, 1.0, -570.0),
  new THREE.Vector3(0.0, 1.0, -597.5),
  new THREE.Vector3(0.0, 1.0, -625.0),
  new THREE.Vector3(0.0, 1.0, -652.5),
  new THREE.Vector3(0.0, 1.0, -680.0)
], false, 'centripetal');

const camTextingToEcosystem = new THREE.CatmullRomCurve3([
  new THREE.Vector3(0.0, 1.0, -680.0),
  new THREE.Vector3(0.0, 1.8, -700.0),
  new THREE.Vector3(0.0, 2.7, -720.0),
  new THREE.Vector3(0.0, 3.6, -740.0),
  new THREE.Vector3(0.0, 4.5, -760.0)
], false, 'centripetal');
...
const SEGMENT_CUTOFFS = [
  ...
  0.57, // Segment 2 cutoff (terminates exactly at the purple cut marker right before the Maison Noir showcase)
...
(function buildWebsites() {
  ...
  const baseMesh = new THREE.Mesh(new THREE.PlaneGeometry(11.0, 260.0), baseMat);
  ...
  const pathwayShaderMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Dim the background emissive strip (frameColor * 0.2) to avoid wash out, boost arrow brightness (arrowColor * 4.0)
        vec3 finalRGB = frameColor.rgb * 0.2 + arrowColor.rgb * 4.0;
        float finalAlpha = max(frameColor.a * 0.6, arrowColor.a * 0.98) * opacity;
  ...
  slabDefs.forEach((d, idx) => {
    ...
    // RESTORE ORIGINAL WEBSITE IMAGES: Use existing IMAGES niche1 - niche6 directly
    const img = IMAGES['niche' + (idx + 1)];
    ...
    const webMat = new THREE.MeshStandardMaterial({
      ...
      emissiveIntensity: 1.0, // High-end self-illumination
      roughness: 0.85,
      metalness: 0.05,
      clippingPlanes: [floorClippingPlane],
      transparent: true,
...
function rebuildEcosystemStreams() {
  ...
  const logoCenter = new THREE.Vector3(0, 4.5, -920);
  ...
  const tlPoints = [
    ...
    new THREE.Vector3(-8, 14.0, -920),

    new THREE.Vector3(-18, 18.0, -920),
  ...
  const blPoints = [
    ...
    new THREE.Vector3(-8, -6.0, -920),

    new THREE.Vector3(-18, -9.0, -920),
...
(function buildConnectionPipe() {
  ...
  const vStart = new THREE.Vector3(0, 4.5, -920);
...
function updateEcosystem(t, time) {
  ...
  if (scene.userData.ecoNodes) {
    ...
    const dist = Math.abs(camera.position.z - (-920));
    ...
    const dynamicPositions = [
      ...
      [-targetX, 4.5 + targetY, -920], // Top Left (Content Creation)

      [targetX, 4.5 + targetY, -920],  // Top Right (Website Experiences)

      [-targetX, 4.5 - targetY, -920], // Bottom Left (AI Calling Agents)

      [targetX, 4.5 - targetY, -920]   // Bottom Right (AI Texting Agents)
...
const goldCurveTravelLimits = [
  ...
  1.00, // Website -> Calling (fully travels end of Segment 3 at Z=-371)

  1.00, // Calling -> Texting (fully travels end of Segment 4 at Z=-472)

  1.00  // Texting -> Ecosystem (fully travels end of Segment 5 at Z=-580)
...
function getSectionOpacity(idx) {
  ...
  // Prevent overlapping titles: Website Experiences fades out early (from progress 0.0 to 0.2)

  // so there is no simultaneous visibility overlap with AI Calling.

  if (idx === 2 && currentSectionIdx === 2 && targetSectionIdx > 2) {

    if (progress < 0.2) return 1.0 - (progress / 0.2);
...
function animate() {
  ...
  function getStationCamera(sectionIdx, webProgress) {
    ...
    if (sectionIdx === 2) {
      ...
      const camZ = lerp(-170.0, -445.0, webProgress);
      const camY = -6.5 + webProgress * 0.55; // subtle 10% eye level elevation increase
  ...
  if (sectionTransitionProgress < 1.0) {
    ...
    if (progressVal < 0.2) {
      ...
      if (minIdx === 1) {
        ...
        limit = 0.58; // Dismount at Z = -185 (Gallery entry)
      ...
      if (toIdx === 2) {
        ...
        // ── 2. NEW DETAILED CINEMATIC BOARDING SEQUENCE (Website → Calling) ──
        // Stage 1 (t_travel = 0.0 to 0.30): Camera remains paused at start position while platform/walkway fades out and tube rises from beneath floor
        // Stage 2 (t_travel = 0.30 to 0.60): Camera physically lowers and tube catches it (boarding target Y = -8.5)
        // Stage 3 (t_travel = 0.60 to 1.00): Camera starts acceleration down the spline tube
        
        // Dynamic rise of Segment 3 visual tube from beneath floor
        if (goldMeshes && goldMeshes[2]) {
          goldMeshes[2].position.y = lerp(-20.0, 0.0, clamp(t_travel / 0.30, 0.0, 1.0));
        }

        const rideHeight = 3.5;
        const tubePosAtStart = followCurve.getPointAt(0.0); // (0.0, -12.0, -445.0)
        const tubeCamPosAtStart = new THREE.Vector3(0.0, -12.0 + rideHeight, -445.0); // (0.0, -8.5, -445.0)
        
        const tangentAtStart = followCurve.getTangentAt(0.0).normalize();
        const tubeLookPosAtStart = tubePosAtStart.clone().addScaledVector(tangentAtStart, 8.0).addScaledVector(new THREE.Vector3(0, 1, 0), -1.0);

        if (t_travel < 0.30) {
          // Stage 1: Pause camera target at the exit of Websites corridor
          finalCamTarget.copy(startState.pos);
          finalLookTarget.copy(startState.look);
        } else if (t_travel < 0.60) {
          // Stage 2: Camera physically lowers from Y=-5.95 to Y=-8.5, aligning perfectly onto the rising tube
          const lowerT = (t_travel - 0.30) / 0.30;
          const easeLower = lowerT * lowerT * (3.0 - 2.0 * lowerT);
          
          finalCamTarget.lerpVectors(startState.pos, tubeCamPosAtStart, easeLower);
          finalLookTarget.lerpVectors(startState.look, tubeLookPosAtStart, easeLower);
        } else {
          // Stage 3: Accelerate forward down the spline tube
          const travelT = (t_travel - 0.60) / 0.40;
          const easedTravelT = travelT * travelT; // quadratic acceleration curve
          const followProgress = easedTravelT * limit;
  ...
  // Target FOV: 48 degrees when in Website section (idx = 2), 68 degrees otherwise
  let targetFov = 68.0;
  if (currentSectionIdx === 2 && targetSectionIdx === 2 && sectionTransitionProgress >= 1.0) {
    targetFov = 48.0;
  } else if (targetSectionIdx === 2 || currentSectionIdx === 2) {
    let progress = sectionTransitionProgress;
    if (targetSectionIdx === 2) {
      targetFov = lerp(68.0, 48.0, progress);
    } else {
      targetFov = lerp(48.0, 68.0, progress);
    }
  }
